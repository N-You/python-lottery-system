...
class User(Base):
    def __init__(self, username, user_json, gift_json):
        ...
        self.get_random = list(range(1, 101))
    ...
    def get_user(self):
        ...
        currnet_user = users.get(self.username)
        if currnet_user.get('active') == False:
            raise UserActiveError('the user %s had not use' % self.username)

        if currnet_user.get('role') != 'normal':
            raise RoleError('permission by normal')

        self.user = currnet_user
        self.name = currnet_user.get('username')
        self.role = currnet_user.get('role')
        self.gifts = currnet_user.get('gifts')
        self.create_time = timestamp_to_string(currnet_user.get('create_time'))
    ...
    def choice_gift(self):
        ...
        level_one_count = random.choice(self.get_random)
        ...
        level_two_count = random.choice(self.get_random)
...
if __name__ == '__main__':
    ...
    user = User('lihua', user_path, gift_path)
    user.choice_gift()
